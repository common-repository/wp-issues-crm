...
class WIC_Function_Utilities { // functions that serve multiple entities	
	...
	public static function wic_error ( $text, $file, $line, $method, $fatal ) {
		...
		echo '<div class="' . $error_class . '">' . ' 
			<p>' . $fatal_phrase . __( $text, 'wp-issues-crm' ) . '</p>' .  
	...
	public static function get_administrator_array() {
		...
		$roles = array( 
			...
			'Contributor' 	=> 4,
			'Subscriber'	=> 3,
			'wic_constituent_manager' => 0,
		) ;
		
		// standard highest capabilities of roles used in definitions of roles in configure>security
		$capability_hierarchy = array (
			'create_sites' 		=> 8,
			'edit_theme_options' => 7,
			'edit_others_posts' => 6,
			'publish_posts'		=> 5,
			'edit_posts'		=> 4,
			'read'				=> 3,
			'manage_wic_constituents' => 0, // constituent manager have this capability 

		);
		
		$wp_issues_crm_access_threshold = WIC_Admin_Access::check_required_capability( '' );
		$minimum_level = $capability_hierarchy[$wp_issues_crm_access_threshold];

		foreach ( $roles as $role => $level ) {

			if ( 'manage_wic_constituents' == $wp_issues_crm_access_threshold ) {
				if ( ! in_array( $role, array( 'wic_constituent_manager', 'Administrator' ) ) ) {
					continue;
				}
			} else {
	...
	public static function get_administrator_array_plus() {
		
		$administrator_array = WIC_Function_Utilities::get_administrator_array();
		array_push ( $administrator_array, array ( 'value' => 'all', 'label' => 'All' ) );
		array_push ( $administrator_array, array ( 'value' => 'any', 'label' => 'All assigned' ) );
		return $administrator_array;
	
	}
	
	// get administrator email array
	public static function get_administrator_email_array() {
	
		$administrator_email_array = array();	
	
		$roles = array( 'Administrator', 'wic_constituent_manager' ) ;
		...
		foreach ( $roles as $role ) {
			...
			$user_list = new WP_User_query ( $user_query_args );
			// consolidate the results into single array	
			foreach ( $user_list->results as $user ) {
				array_push ( $administrator_email_array, $user );								
			} 
		}

		return ( $administrator_email_array );
	...
	public static function issue_staff_formatter ( $user_id ) {
		$display_name = '';		
		if ( $user = WP_User::get_data_by( 'id', $user_id ) ) {
			$display_name = $user->display_name; 
		} else {
			$display_name = 'Assigned staff deleted';
		}
		return ( $display_name );
	}
	...
	* 
	*/
	public static function value_label_lookup_cold ( $value, $option_group ) {
		
 		global $wic_db_dictionary;
		$options_array = $wic_db_dictionary->lookup_option_values( $option_group );
		if ( ! is_array ( $options_array ) || ! $options_array ) {
			return false;
		} else {
			return self::value_label_lookup ( $value, $options_array );
		}
	...
	*/
	...
	public static function null_list () {
		return ( array () );
	}

	/*
	* WIC_Function_Utilities::wic_sanitize_file_name
	*
	...
	*/
	public static function wic_sanitize_file_name ( $filename ) {
	...
	*/
	...
	public static function wic_mime_content_type_from_filename( $filename ) {
		/*
